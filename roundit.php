...
/*
Plugin Name: Roundit
Plugin URI: http://itmine.com/roundit/
Description: This plugins allows to easily add roundness to your wordpress images alongwith border and shadow
Version: 1.0
...
/**
*  function to generate roundness field
* @return form_fields array
*/


function quick_image_roundness( $form_fields, $post ) {
...
add_filter( 'attachment_fields_to_edit', 'quick_image_roundness', 10, 2 );
...
function quick_image_roundness_save( $attachment_id ) {
...
add_action( 'edit_attachment', 'quick_image_roundness_save' );
...
function quick_image_add_style($html, $id, $caption, $title, $align, $url, $size, $alt = '' ){
...
box-shadow: 0px 0px '.$shadow_size.' '.$shadow_blur.' #000 ;border:'.$border.' '.$border_color.' '.$solid.' ;border-radius:'.$location.'; behavior:url('.plugins_url().'/roundit/pie/PIE.php); position:relative; '. get_post($id)->post_excerpt.'" /></a>',$html);
	// returning new link
	return $html;
}
...
*/
add_filter('image_send_to_editor','quick_image_add_style',10,8);
?>
